    ...
    <div class="container-fluid">
        <section class="section card mb-5">
            <div class="card-body">
                ...
                <h1 class="text-center my-5 h1">Actualizar Tutor</h1>
                <?php
                    ...
                    <div class="row">
                        <div class="col">
                            <select name="alumno" required class="mdb-select colorful-select dropdown-primary md-form" searchable="Buscar..">
                                ...
                                ?>
                                            <option <?php if ($alumno_id == $id_alumno) {
                                                        echo "selected";
                                                    } ?> value="<?php echo $id_alumno; ?>">Matricula: #<?php echo $matricula . " | Nombre: " . $nombre_alumno . " " . $apellido_alumno; ?></option>
                                <?php }
                                    }
                                }
                                ?>
...
<?php if (isset($_POST['submit'])) {
    if (isset($_POST['id']) && isset($_POST['nombre']) && isset($_POST['apellido']) && isset($_POST['email']) && isset($_POST['reEmail']) && isset($_POST['telefono']) && isset($_POST['alumno'])) {
        $id = $_POST['id'];
        $nombre = $_POST['nombre'];
        ...
        if (!$soloLetras1 || !$soloLetras2) {
            ...
            $_SESSION['id'] = $id;
            header("location:" . SITEURL . 'admin/actualizar_tutor.php');
            die();
        }
        ...
        $sql = "SELECT * FROM tutor WHERE email = '$email' AND activo = 1 AND id_tutor != $id";
        ...
        if ($res == TRUE) {
            unset($_SESSION['id']);
            $_SESSION['actualizar'] = "<div class='alert alert-success'>Tutor Actualizado Correctamente</div>";
            header("location:" . SITEURL . 'admin/consultar_tutor.php');
            die();
        } else {
            $_SESSION['id'] = $id;
