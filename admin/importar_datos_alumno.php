        ...
        <section class="section card mb-5">
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <a class="btn verde-conalep float-right waves-effect waves-light" href="formatos/Alumno.xlsx" role="button">
                            <i class="fas fa-file-download mr-2"></i>
                            Generar Archivo para Carga
                        </a>
                    </div>
                </div>
                <h1 class="text-center my-5 h1">Importar Datos Alumno</h1>
                <?php
                ...
                ?>
                <form action="importar_datos_alumno_2.php" method="POST" enctype="multipart/form-data">
                    <div class="row mb-4">
                        <div class="col">
    ...
    if (isset($_FILES['file']['name']) && in_array($_FILES['file']['type'], $file_mimes)) {
        ...
        if (!empty($sheetData)) {
            ...
            for ($i = 2; $i < count($sheetData); $i++) { //skipping first row
                ...
                $fecha_ingreso = $sheetData[$i][6];
                $fecha_nacimiento = $sheetData[$i][7];
                $carrera = $sheetData[$i][8];

                $sql = ("SELECT * FROM alumno WHERE activo = 1 AND (email = '$email') OR (matricula = '$matricula')");
                $res = mysqli_query($conn, $sql) or die(mysqli_error($conn));
                $count = mysqli_num_rows($res);
                if ($count > 0) {
                    $no_agregado = $count + $no_agregado;
                } else {
                    $sql2 = ("INSERT INTO alumno (nombre, apellido, matricula, email, password, telefono, fecha_ingreso, fecha_nacimiento, carrera_id, rol_id,activo)VALUES('$nombre', '$apellido',$matricula, '$email','$password', '$telefono', '$fecha_ingreso', '$fecha_nacimiento', $carrera, 3, 1)");
