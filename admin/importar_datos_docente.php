        ...
        <section class="section card mb-5">
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <a class="btn verde-conalep float-right waves-effect waves-light" href="formatos/Docente.xlsx" role="button">
                            <i class="fas fa-file-download mr-2"></i>
                            Generar Archivo para Carga
                        </a>
                    </div>
                </div>
                <h1 class="text-center my-5 h1">Importar Datos Docente</h1>
                <?php
                ...
                ?>
                <form action="importar_datos_docente_2.php" method="POST" enctype="multipart/form-data">
                    <div class="row mb-4">
                        <div class="col">
    ...
    if (isset($_FILES['file']['name']) && in_array($_FILES['file']['type'], $file_mimes)) {
        ...
        if (!empty($sheetData)) {
            ...
            for ($i = 1; $i < count($sheetData); $i++) { //skipping first row
                ...
                $numero_empleado = $sheetData[$i][2];
                $email = $sheetData[$i][3];
                $password = $sheetData[$i][4];
                $password = password_hash($password, PASSWORD_DEFAULT);
                $sql = ("SELECT * FROM docente WHERE activo = 1 AND (email = '$email') OR (numero_empleado = $numero_empleado)");
                $res = mysqli_query($conn, $sql) or die(mysqli_error($conn));
                $count = mysqli_num_rows($res);
                if ($count > 0) {
                    $no_agregado = $count + $no_agregado;
                } else {
                    $sql2 = ("INSERT INTO docente (nombre, apellido,numero_empleado, email, rol_id, password, activo)VALUES('$nombre', '$apellido',$numero_empleado, '$email', 2, '$password', 1)");
        ...
        header("location:" . SITEURL . 'admin/importar_datos_docente.php');
    }
}
