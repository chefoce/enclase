    ...
    <div class="container-fluid">
        <section class="section card mb-5">
            ...
            <div class="row">
                    ...
                    <div class="col-8">
                        <a class="btn verde-conalep float-right waves-effect waves-light" href="importar_datos_alumno_grupo.php" role="button">
                            <i class="fas fa-file-excel mr-2"></i>
                            Importar
                        </a>
                    </div>
                </div>
                <h1 class="text-center my-5 h1">Asignar Grupo Alumno</h1>
                <?php
...
if (isset($_POST['submit'])) {
    ...
    $sql = ("SELECT * FROM alumno WHERE id_alumno = '$id_alumno' AND grupo_id = $id_grupo AND activo = 1");
    $res = mysqli_query($conn, $sql) or die(mysqli_error($conn));
    $count = mysqli_num_rows($res);
    if ($count > 0) {
        $_SESSION['error'] = "<div class='alert alert-danger'>El Alumno ya tiene grupo Asignado</div>";
        header("location:" . SITEURL . 'admin/agregar_alumno_grupo.php');
    } else {
        $sql2 = "UPDATE alumno SET grupo_id = $id_grupo WHERE id_alumno = $id_alumno";
        $res2 = mysqli_query($conn, $sql2) or die(mysqli_error($conn));

        if ($res2 == TRUE) {
            $_SESSION['agregar'] = "<div class='alert alert-success'>Grupo Asignado Correctamente</div>";
            header("location:" . SITEURL . 'admin/consultar_alumno.php');
        } else {
            $_SESSION['error'] = "<div class='alert alert-danger'>Error al Asignar</div>";
            header("location:" . SITEURL . 'admin/agregar_alumno_grupo.php');
        }
    }
}
