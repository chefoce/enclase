        ...
        <section class="section card mb-5">
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <a class="btn verde-conalep float-right waves-effect waves-light" href="exportar_datos_grupo_alumno.php" role="button">
                            <i class="fas fa-file-download mr-2"></i>
                            Generar Archivo para Carga
                        </a>
                    </div>
                </div>
                <h1 class="text-center my-5 h1">Importar Datos Grupo Alumno</h1>
                <?php
    ...
    if (isset($_FILES['file']['name']) && in_array($_FILES['file']['type'], $file_mimes)) {
        ...
        if (!empty($sheetData)) {
            ...
            for ($i = 2; $i < count($sheetData); $i++) { //skipping first row
                ...
                $sql = ("SELECT * FROM alumno_grupo WHERE alumno_id = '$alumno_id' AND activo = 1");
                $res = mysqli_query($conn, $sql) or die(mysqli_error($conn));
                $count = mysqli_num_rows($res);
                if ($count > 0) {
                    $no_agregado = $count + $no_agregado;
                } else {
                    $sql2 = ("INSERT INTO alumno_grupo(alumno_id, grupo_id, activo)VALUES('$alumno_id', '$grupo_id', 1)");
        ...
        header("location:" . SITEURL . 'admin/importar_datos_alumno_grupo.php');
    }
}
