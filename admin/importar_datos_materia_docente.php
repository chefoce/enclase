        ...
        <section class="section card mb-5">
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <a class="btn verde-conalep float-right waves-effect waves-light" href="exportar_datos_materia_docente.php" role="button">
                            <i class="fas fa-file-download mr-2"></i>
                            Generar Archivo para Carga
                        </a>
                    </div>
                </div>
                <h1 class="text-center my-5 h1">Importar Datos Materia Docente</h1>
                <?php
    ...
    if (isset($_FILES['file']['name']) && in_array($_FILES['file']['type'], $file_mimes)) {
        ...
        if (!empty($sheetData)) {
            ...
            for ($i = 2; $i < count($sheetData); $i++) { //skipping first row
                $docente_id = $sheetData[$i][0];
                $grupo_id = $sheetData[$i][1];
                $materia_id = $sheetData[$i][2];
                $periodo_id = $sheetData[$i][3];
                $sql = ("SELECT * FROM docente_materia WHERE docente_id = '$docente_id' AND grupo_id = '$grupo_id' AND materia_id = '$materia_id' AND periodo_id = '$periodo_id' AND activo = 1");
                $res = mysqli_query($conn, $sql) or die(mysqli_error($conn));
                $count = mysqli_num_rows($res);
                if ($count > 0) {
                    $no_agregado = $count + $no_agregado;
                } else {
                    $sql2 = ("INSERT INTO docente_materia(docente_id, grupo_id, materia_id, periodo_id,activo)VALUES('$docente_id', '$grupo_id','$materia_id','$periodo_id' , 1)");
        ...
        header("location:" . SITEURL . 'admin/importar_datos_materia_docente.php');
    }
}
