    ...
    <div class="container-fluid mb-5">
        <section>
            <div class="col-md-12">
                <div class="row">
                    <div class="col">
                        <a href="consultar_alumnos.php"><i class="my-4 fas fa-arrow-circle-left btn-atras"></i></a>
                    </div>
                ...
                <div class="row">
                    ...
                    if ($res == TRUE) {
                        ...
                        if ($count == 1) {
                            ...
                            header('location:' . SITEURL . 'docente/consultar_alumnos.php');
                        }
                    }
                    ?>
                    <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            <select name="materia" required class="mdb-select colorful-select dropdown-primary md-form">
                                ...
                                $sql = "SELECT dm.id_dm,
                                        ...
                                        WHERE dm.docente_id = $id_docente AND ad.alumno_id = $id_alumno AND dm.activo = 1";
                                $res = mysqli_query($conn, $sql);
                                if ($res == TRUE) {
                                    $count = mysqli_num_rows($res);
                                    if ($count > 0) {
                                        ?> <option value="" disabled selected>Selecciona la Materia</option> <?php
                                        while ($rows = mysqli_fetch_assoc($res)) {
                                            $id_dm = $rows['id_dm'];
                                            $materia = $rows['materia'];
                                ?>
                                            <option value="<?php echo $id_dm; ?>"><?php echo $materia ?></option>
                                <?php
                                        }
                                    }else{
                        ...
                        <table id="exportarDatos" class="table table-striped" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                ...
                                if (isset($_POST['submit'])) {
                                    ...
                                    $fecha_i = date('Y-m-d', $fecha_inicial);       
                                    $fecha_final = $_POST['fecha_final'];
                                    $fecha_final = strtotime($fecha_final);
                                    $fecha_f = date('Y-m-d', $fecha_final);
                                    $sql = "SELECT  m.nombre AS materia,
                                            ...
                                            JOIN materia m ON dm.materia_id = m.id_materia
                                            WHERE a.alumno_id = $id_alumno AND a.dm_id = $dm_id AND a.fecha BETWEEN '$fecha_i' AND '$fecha_f' ORDER BY a.fecha DESC";
                                } else {
                                    $sql = "SELECT  m.nombre AS materia,
                                                m.semestre,
                                                a.asistencia,
...
<script>
  $('#exportarDatos').DataTable({
    ...
    buttons: [{
        ...
        messageBottom: 'Reporte Generado <?php echo date('d-m-Y'); ?> -- Hora: <?php echo date("H:i:s"); ?> '
      },
      {
        extend: 'pdfHtml5',
